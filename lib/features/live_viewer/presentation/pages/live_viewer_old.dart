...
import 'package:moonlight/features/live_viewer/presentation/widgets/guest_video_layout.dart';
          ...
          child: SafeArea(
            ...
            child: Stack(
              children: [
                ...
                // Guest video layout (appears when there's a guest)
                const GuestVideoLayout(),
...
class _BackgroundVideo extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // Prefer the repo used by the bloc (if present)
    final bloc = context.read<ViewerBloc>();
    final repoToUse = bloc.repo ?? repository; // Handle null case
    ...
    return BlocBuilder<ViewerBloc, ViewerState>(
      ...
      builder: (context, s) {
        ...
        // When in two-up mode, show black background
        if (anyGuest) {
          return Stack(
            fit: StackFit.expand,
            children: [
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<ViewerBloc, ViewerState>(
      ...
      builder: (context, state) {
        // Show 0 only if it's actually 0, not from empty events
        final displayCount = state.viewers < 0 ? 0 : state.viewers;
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<ViewerBloc, ViewerState>(
      ...
      builder: (_, s) {
        ...
        debugPrint(
          '🎯 TwoUpSwitch Debug: iAmGuest=$iAmGuest, anyGuest=$anyGuest, '
          'activeGuestUuid=${s.activeGuestUuid}, isPaused=${s.isPaused}',
        );

        // Only show two-up layout when there's actually a guest
        if (!anyGuest) {
          debugPrint('🎯 TwoUpSwitch: No guest, returning empty');
          return const SizedBox.shrink();
        }
        ...
        debugPrint('🎯 TwoUpSwitch: Building layout with height $videoHeight');
            ...
            Positioned(
              ...
              child: Container(
                ...
                child: Stack(
                  fit: StackFit.expand,
                  children: [
                    // ADD NULL CHECK FOR buildHostVideo
                    vp.buildHostVideo(),
                    // Debug overlay
                    Positioned(
                      top: 10,
                  ...
                  children: [
                    iAmGuest
                        ? (vp.buildLocalPreview() ??
                              _buildVideoPlaceholder('YOU'))
                        : vp.buildGuestVideo() ??
                              _buildVideoPlaceholder('GUEST'), // ADD NULL CHECK
                    // Debug overlay
                    Positioned(
                      top: 10,
                      left: 10,
...
class _LiveViewerPagerState extends State<LiveViewerPager> {
  ...
  void _onPageChanged(int idx) {
    ...
    // Clean up repos that are far away
    final toRemove = <int>[];
    for (final key in _repoCache.keys) {
      if ((key - idx).abs() > 2) {
        // Keep 2 pages ahead/behind
        try {
          _repoCache[key]?.dispose();
        } catch (_) {}
        toRemove.add(key);
      }
    }
    for (final key in toRemove) {
      _repoCache.remove(key);
    }
  }
    ...
    return Scaffold(
      ...
      body: PageView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return BlocProvider(
            create: (_) {
              final bloc = ViewerBloc(repo);
              // Small delay to ensure UI is ready
              Future.delayed(const Duration(milliseconds: 100), () {
                if (!bloc.isClosed && mounted && !_disposed) {
                  bloc.add(const ViewerStarted());
                }
              });
              return bloc;
...
// Add to your LiveViewerScreen stack:
...
class _NetworkDiagnostics extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // First check if repo is not null, then check type
    ...
    // Check if vp is not null AND is AgoraViewerService
    if (vp == null || vp is! AgoraViewerService) return const SizedBox.shrink();

    // Now safely cast to AgoraViewerService
    final agoraService = vp as AgoraViewerService;
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 4),
            Text(
              'Host Video: ${agoraService.hostHasVideo.value}',
              style: TextStyle(
                color: agoraService.hostHasVideo.value
                    ? Colors.green
                    : Colors.red,
                fontSize: 10,
              ),
            ...
            Text(
              'Guest Video: ${agoraService.guestHasVideo.value}',
              style: TextStyle(
                color: agoraService.guestHasVideo.value
                    ? Colors.green
                    : Colors.red,
                fontSize: 10,
              ),
            ...
            Text(
              'Joined: ${agoraService.isJoined}',
              style: const TextStyle(color: Colors.white, fontSize: 10),
            ),
            Text(
              'CoHost: ${agoraService.isCoHost}',
              style: const TextStyle(color: Colors.white, fontSize: 10),
            ),
          ],
        ),
