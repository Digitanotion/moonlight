...
class InAppUpdatePlugin : FlutterPlugin, MethodCallHandler,
    ...
    // Was a bare status Int — now the full state, so the Dart side can show
    // real download progress (a ring around the logo) instead of just a
    // coarse "downloading vs. downloaded" flag. bytesDownloaded/
    // totalBytesToDownload are 0 for any status where Play doesn't report
    // them (e.g. before downloading starts), which the Dart side treats as
    // "no progress fraction yet" rather than dividing by zero.
    private fun addState(installState: InstallState) {
        installStateSink?.success(
            mapOf(
                "status" to installState.installStatus(),
